...
def should_check_pair(node1: Dict, node2: Dict) -> bool:
    ...
    # Frame/Image는 제외
    if type1 in ['Frame', 'Image'] or type2 in ['Frame', 'Image']:
        return False
    
    # Decoration/Marker 조합만 검사
    checkable_roles = ['Decoration', 'Marker']
    if role1 in checkable_roles and role2 in checkable_roles:
        return True
...
def main():
    ...
    result = {
        "metadata": {
            ...
            "statistics": {
                "total_filtered_agentic": total_count,
    ...
    print(f"\n필터링된 agentic (depth {MIN_DEPTH}~{MAX_DEPTH}, role/signature 필터 적용): {total_count:,}개")
    print(f"변경 대상:    {len(candidates):,}개 ({len(candidates)*100/total_count:.2f}%)" if total_count > 0 else "변경 대상:    0개")
    print(f"\n📁 저장 위치: {output_path}")
