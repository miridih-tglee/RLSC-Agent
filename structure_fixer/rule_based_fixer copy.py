...
# 설정
# ============================================================
INPUT_PATH = "/Users/miridih/Desktop/tg/structure_fixer/samples/sample_277987.json"
OUTPUT_PATH = "/Users/miridih/Desktop/tg/structure_fixer/samples/sample_277987_r.json"
...
def is_decoration(node: Dict) -> bool:
    return get_role(node) == 'Decoration'
...
def fix_node(node: Dict, depth: int = 0) -> Dict:
    ...
    # 3. 가장 큰 Decoration을 Background로 변경 (배경 역할)
    #    이렇게 하면 이 요소는 겹침 검사에서 제외됨
    ...
    if bg_candidate_idx >= 0:
        ...
        print(f"{indent}   🎨 배경 후보 → Background: {bg_candidate.get('id', 'unknown')[:20]}")
    
    # 4. 나머지 중 겹치는 쌍 찾기 (Background는 제외됨)
